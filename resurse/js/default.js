(function (global) {
    ...
    app.articles.ShortPost = function (post, target) {
        var dateArray = post.create_date.split(" ");//$('<div></div>').text(post.url_path).html()
        var date = dateArray[0];
        var time = dateArray[1];
        var urlPath = '/' + date + '/' + time.replace(/:/g, '-') + '/' + $('<div></div>').text(post.url_path).html();
        var shorts = $('<div class="post"><h3><a href="' + urlPath +
                        '">' +
